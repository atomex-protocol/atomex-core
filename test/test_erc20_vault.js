const AtomexErc20Vault = artifacts.require('./contracts/ethereum/AtomexErc20Vault.sol');
const FiatTokenV1 = artifacts.require('./contracts/ethereum/FiatTokenV1.sol');
...
contract('AtomexErc20Vault', async (accounts) => {
    let contractSwap;
    let contractUSDC;
    ...
    beforeEach(async function(){
        contractSwap = await AtomexErc20Vault.new();
        contractUSDC = await FiatTokenV1.new(name, symbol, supply, owner);
    });
    ...
    it('should not intitiate with wrong refund timestamp', async () => {
        ...
        try {
            ...
            assert(error.message.indexOf('refundTimestamp has already come') >= 0, error.message);
        }
